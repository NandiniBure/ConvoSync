...
export const SelfGameModel = () => {
  const router = useRouter();
  const [isMounted, setIsMounted] = useState(false);
  const { isOpen, onClose, aiChatResult, type } = useModal();
  const isModelOpen = isOpen && type === "selfgameModel";
      ...
      <Dialog open={isModelOpen} onOpenChange={onClose}>
        ...
        >
          <DialogHeader className="pt-4 px-6">
            ...
            <h1 className=" text-center text-white font-bold text-xl">Single Player</h1>
            <span><Gamepad2Icon className=" w-8 h-8 text-white ml-5"></Gamepad2Icon></span>
            </div>
